          ...
          out indices  uint3      a_primitiveList[k_maxMeshletPrimitiveCount])
{
    ...
    for (uint l_vertexIndex = 0U; l_vertexIndex < l_modelMeshlet.vertexCount; ++l_vertexIndex)
    {
        ...
        const float4 l_localPosition = float4(l_modelVertex.position, k_maxMeshletPrimitiveCount);
        const float4 l_worldPosition = mul   (l_localPosition,        g_worldMatrix);
        
        const float3x3 l_worldMatrix3x3 = (float3x3)g_worldMatrix;
        ...
        a_vertexList[l_vertexIndex].normal        = normalize(mul(l_modelVertex.normal, l_worldMatrix3x3));
