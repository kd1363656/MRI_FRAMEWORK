...
namespace FWK::Utility::TypeInfo
{
	...
	inline bool IsDerivedFrom(const FWK::TypeInfo& a_typeInfo, const FWK::TypeInfo& a_baseTypeInfo)
	{
		...
		// ”O‚Ì‚½‚ßƒkƒ‹ƒ`ƒFƒbƒN(ŽQÆ‚È‚Ì‚Å"nullptr"‚Íâ‘Î‚É‚ ‚è‚¦‚È‚¢‚ª)
		if (!l_currentInfo)
		{
		...
		// Ž©g‚ª"nullptr"‚ðŽ¦‚·‚Â‚Ü‚èe‚ð‘–¸’ª‚ð“¾‚½‚ç"while"‚ð”²‚¯‚é
		while (l_currentInfo)
		{
			// Ž©g‚ÌŒ»Ý‚Ì"ID"‚Æe‚Ì"ID"‚ªˆê’v‚·‚ê‚Î"true"‚ð•Ô‚·‚·
			if (l_currentInfo->k_id == a_baseTypeInfo.k_id)
			{
				return true;
			}

			// Ž©g‚Ìƒƒ“ƒo‚ÉŠÜ‚Ü‚ê‚ée•ƒ‚ðŽŸ‚Ì"while"‚Å‘–¸‚·‚é‘ÎÛ‚Æ‚µ‚ÄŽw’è(Œp³ƒcƒŠ[‚ð’H‚é)
