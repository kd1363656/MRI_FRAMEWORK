...
void FWK::Window::Init()
{
	...
	m_windowConfig.styleTag = Utility::Tag::GetTag<Tag::WindowStyleNormalTag>();

	m_hwnd = nullptr;
}
bool FWK::Window::Create(const std::string& a_titleName, const std::wstring& a_windowClassName)
{
	if (!CreateWindowInstance(a_titleName, a_windowClassName))
	{
...
void FWK::Window::LoadConfig()
{
	...
	m_windowJsonConverter.Deserialize(l_json);
}
void FWK::Window::SaveConfig()
{
	const auto& l_json = m_windowJsonConverter.Serialize();
...
bool FWK::Window::CreateWindowInstance(const std::string& a_titleName, const std::wstring& a_windowClassName)
{
	// ハンドル(.exe)を取得
	const HINSTANCE& l_hInstance = FetchInstanceHandle();
	...
	const DWORD l_style = FetchWindowStyle();
...
void FWK::Window::SetClientSize()
{
	...
	if(m_windowConfig.styleTag == Utility::Tag::GetTag<Tag::WindowStyleFullScreenTag>())
	{
		// 現在の画面解像度を取得する
	...
	else
	{
		...
		MoveWindow(m_hwnd,
				   l_rcWND.left,
				   l_rcWND.top,  
				   static_cast<int>(m_windowConfig.width)  + l_frameWidth, 
				   static_cast<int>(m_windowConfig.height) + l_frameHeight,
				   TRUE);
	}
}

HINSTANCE FWK::Window::FetchInstanceHandle() const
{
...
DWORD FWK::Window::FetchWindowStyle() const
{
	// 持っているタグから返すウィンドウスタイルを判定する
	if (m_windowConfig.styleTag != Tag::WindowStyleNormalTag::GetTypeInfo().k_id)
	{
		return WS_POPUP;
	}
