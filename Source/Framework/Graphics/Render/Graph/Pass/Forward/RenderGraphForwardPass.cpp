...
FWK::Graphics::RenderGraphForwardPass::RenderGraphForwardPass()
{
	...
	// 現段階ではSceneDrawPassの書き換えなのでDepthWriteにしておく
														  ...
														  RenderGraph&						 a_renderGraph)
{
	...
	// RenderGraph側でSceneColorTextureはD3D12_RESOURCE_STATE_TARGETへ、
	...
	a_directCommandList.SetupDescriptorHeap(a_srvDescriptorHeap.GetREFDescriptorHeap());
	...
	if (const auto& l_staticModelStandardpathPerObjectDrawRequestUnLit = a_renderGraph.FindVALDrawRequestPerObject<StaticModelStandardPerObjectDrawRequestUnLit>().lock())
	{
		// UnLitモデルをForwardで描画する。
		// ライト計算をしないモデルや、デバッグ表示などに使う
		l_staticModelStandardpathPerObjectDrawRequestUnLit->RequestForwardDraw(a_textureSystem, a_renderer);
	}
}