...
{
	const auto& l_pipelineStateTag = Utility::Tag::GetVALTag<Tag::SpriteStandardPipelineStateTag>();
	const auto& l_rootSignature    = SetGraphicsPipelineStateAndFetchRootSignature				 (a_renderer, l_pipelineStateTag).lock();

	FWK_ASSERT_RETURN_IF(!l_rootSignature, "Forward描画用RootSignatureが無効なため、Sprite描画処理に失敗しました。")
