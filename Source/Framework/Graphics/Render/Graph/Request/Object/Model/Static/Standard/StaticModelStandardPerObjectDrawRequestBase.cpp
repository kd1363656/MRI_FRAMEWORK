																							  ...
																							  const Struct::ModelMesh&									 a_modelMesh)
{
	...
	const auto& l_modelMeshRuntimeData     = a_modelMesh.m_modelMeshRuntimeData;
	...
	// モデル1体ごとの行列
	l_cbModelPerObject.m_worldMatrix = a_staticModelStandardPerObjectDrawRequestDataList.m_worldMatrix;
	...
	// Textureがない場合はDefaultTextuerを使う
...
bool FWK::Graphics::StaticModelStandardPerObjectDrawRequestBase::TransitionMaterialTexture(const DirectCommandList& a_directCommandList, const TextureSystem& a_textureSystem, const Struct::ModelMesh& a_modelMesh) const
{
	...
	FWK_ASSERT_RETURN_VALUE_IF(!l_isBaseColorTextureSuccess, "NormalTextureの状態遷移に失敗しました。", false)
