...
{
	const auto& l_copyCommandList = GetCommandList();

	if (!l_copyCommandList)
	{
		assert(false && "CopyCommandListが作成されておらず、テクスチャコピー処理に失敗しました。");
		return;
	}
	...
	//			　 SUBRESSOURCE_INDEXなら「何番目のサブリソースか」
	//			   PLACED_FOOTPRINTなら「UploadBuffe上のどこに画像があるか」

	// どのリソースのどこへ、どのリソースのどこからコピーするかを指定する
	// CopyTextureRegin(コピー先の情報、
	//					コピー先X座標、
	//					コピー先Y座標、
	//					コピー先Z座標,
	//					コピー元の情報、
