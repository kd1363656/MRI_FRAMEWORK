...
bool FWK::Graphics::ShaderCompiler::Create()
{
	...
	FWK_ASSERT_RETURN_VALUE_IF(FAILED(l_hr), "DxcUtilsの作成に失敗しました。", false)
	...
	FWK_ASSERT_RETURN_VALUE_IF(FAILED(l_hr), "DxcCompilerの作成に失敗しました。", false)
...
{
	FWK_ASSERT_RETURN_VALUE_IF(!m_dxcUtils, "DxcUtilsの作成がされておらず、シェーダーバイナリファイルの読み込みができませんでした。", nullptr)
	...
	FWK_ASSERT_RETURN_VALUE_IF(FAILED(l_hr), "IDxcBlobEncodingからIDxcBlobへの変換に失敗しました。", nullptr)
